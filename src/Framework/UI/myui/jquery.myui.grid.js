    ...
    $.widget("myui.grid", {
        ...
        */ 
        options: {
            ...
            config: null, //config of page,為了在client能簡單設定Config，與PCM分開，其對應pcm中的element.cfg段
            pcm: null, //pcm of page object
            columns: [], //columns [{name:'#ID',type:'string',length:20,}]
        ...
        * @property version {jQuery Object}
        */ 
        ,LeftHeader: "1.0.0"
        ...
        */
        , adjust: function(weight,height) {
            ...
            var w = (typeof weight=='undefined')?this.element.parent().width():weight
            , h = (typeof weight=='undefined')?this.element.parent().height():height
            , h_top = 0
            , h_bottom = 0
            , h_title = 0;
    ...
    */        
	, addRow: function(event) {
	    ...
	    if (typeof event != 'undefined' && typeof event.values != 'undefined') {
	        data = $.myui.request('a', event.values, grid.options.url);
	    }else{
	    	data = $.myui.request('a', {}, grid.options.url);
	    }
	    grid.bind(data, true);
...
//	    }
    ...
    * @param ArgIsFullName {Bool} ID是否为全称
    */        
	, setCond: function(values, ArgIsFullName) {
	    var tp = 'updcond', idPrefix = this.element.attr("id") + '-cond-', id,
	    data = { kind: 'cond', fld: '', value: '', rowid: '' },
	    isFullName = false;
	    if (typeof ArgIsFullName == 'undefined') {
	        isFullName = false
	    } else {
	        isFullName = ArgIsFullName;
	    }
	    if (isFullName) {
	        id = '';
	    } else {
	        id = this.element.attr("id") + '-cond-';
	    }
	    //alert(this._gridId());
	    for (name in values) {
	        if (isFullName) {
	            data.fld = name.substr(idPrefix.length);
	        } else {
	            data.fld = name;
	        }
	        data.value = (values[name]==null?'':values[name]);
	        $('#' + id + name).val(data.value);
	        //$.myui.request(tp, data, this.options.url);
	    };
	    $.myui.request(tp, values, this.options.url);
    ...
    */        
	, query: function(event) {
	    ...
	    pagePages,
	    grid = ((typeof event != 'undefined' && typeof event.data != 'undefined' && typeof event.data.grid != 'undefined') ? event.data.grid : this),
	    pageSize = grid.options.config.pagesize, //每页笔数
	    data ,
	    fields='';
        ...
        if(typeof event === 'object' && typeof event.data ==='undefined'){
            for(var name in event){
                //alert(event[name]);
                sendData[name] = event[name];
            }
        
        }
	    
	    if(grid.options.config == null){
	        data = $.myui.request('q', sendData, grid.options.url);
	    }else{
            $.each( grid.options.columns, function(i, n){
	        ...
	        data = $.myui.invoke(grid.options.className+".Query", event, {myui_fields:fields});
	    }
	    ...
	    if (window.confirm($.myui.lang('cfm_del'))) {
	        ...
	        if (!(data == "" || data == null)) {
	            ...
	            for (var m = 0; m < rows.length; m++) {
	                //alert(typeof $(rows[m]).children().first().find('input').attr('checked'));
	                if ($(rows[m]).children().first().find('input').attr('checked') == true) {
    ...
    */        
	, save: function(event) {
	    ...
	    //资料有异动
	    if (grid.isChanged()) {
	        ret = $.myui.request('s', {}, grid.options.url);
    ...
    */        
	, setValue: function(values,id) {
	    var data = { kind: 'data', fld: '', value: '', rowid: '' }
	    , row = this.currRow()
	    , rowid;
	        ...
	        for (var m = 0 ;m<=1;m++){
	            if (row[m].find('td[name="' + name + '"]').length == 1) {
                    ...
                    if(row[m].find('td[name="' + name + '"]').find('#editor').length==1){
	                    row[m].find('td[name="' + name + '"]').find('#editor').val(data.value);
	                }
	                //排除CheckBox
	                if(row[m].find('td[name="' + name + '"]').find('span').find('input').length==0){
	                    row[m].find('td[name="' + name + '"]').find('span').text(data.value);
	                }
	            }
	        }
	    ...
	    $.myui.request('u', values, this.options.url);
    ...
    * 設定當前Grid的提示信息
    * @method setMsg
    * @param message {Array} 一个或多个栏的样式设定对象，如：[{id:qty,name:'數量：',value:200},{id:amt,name:'金額（RMB）：',value:20020.23}]
    *
               ...
               case 'object':
                    if($.isArray(message)){
	                    for (var m =0 ;m< message.length;m++){
	                        msg += message[m].name +':' + message[m].value + ";";
	                    }
	                }else{
    	                msg += message.name + ':'+ message.value + ";";
	                }
    	            break;
            ...
            for (var m = 0; m < records.length; m++) {
                ...
                for (var i = 0; i < this.options.columns.length; i++) {
                    ...
                    if (record[this.options.columns[i].id] == null){
                        ...
                        val = $.myui.safeStr(record[this.options.columns[i].id]);
                    }
                    
                    if (typeof this.options.columns[i].checkbox != 'undefined') {
                        align = 'center';
                        if (val == this.options.columns[i].checkbox.checked) {
...
//                    }
            ...
            */            
                    td = '<td name="' + this.options.columns[i].id + '" readonly="' + read + '" id="' + this._gridId() + '-' + m + '-' + this.options.columns[i].id + '" style="width:' + w + ';display:' + display + ';text-align:' + align + ';'+ tdStyle + '"  ><span name="val">' + val + '</span></td>';
                    tdStyle='';
                    ...
                    if (i >= this.options.fixCols) {//data right
                        ...
                        if (typeof this.options.columns[i].iskey != 'undefined' && this.options.columns[i].iskey == '1') {
                ...
                tr_r = '<tr pid="' + pid + '" new="' + mk_new_row + '" class="ui-widget-content '+className+'" style="height:21px;display:' + showRow + ';'+trStyle+'">' + td_r + '</tr>';
                tr_l = '<tr pid="' + pid + '"  new="' + mk_new_row + '"  class="ui-widget-content '+className+'" style="height:21px;display:' + showRow + ';'+trStyle+'">' + td_l + '</tr>';
            ...
            //left data
            objTD = $(data_l + ' tbody tr[new="' + mk_new_row + '"]').find('td[name!="cell0"]');
            ...
            objTD = $(data_r + ' tbody tr[new="' + mk_new_row + '"]').find('td[name!="cell0"]');
            objTD.unbind(grid.options.editorTrigger);
    ...
    *  调用时，注意前面加'grid'并全部用小写，eg. grid.bind('gridrowselected',{},function(event){alert(event.data);});
    * @event rowselected
    * @param event {Object} 父事件的参数
    */        
    , _rowselected: function(event) {

        this._trigger("rowselected", event);
    }
    /** 
    * 调整行高、列宽
    ...
    */          
    , _selectRow: function(event) {
        ...
        checked_count = data_id.find('table>tbody td[name="cell0"]>span>input[type="checkbox"][checked=true]').length;
    ...
    */        
    , _selectedPage: function(val) {
        if(val){
            ...
            $('#' + this._gridId() + '-data-left tbody td[name="cell0"]').find('input[checked=false]').attr('checked', true);
        }else{
            //$('#' + this._gridId() + '-data-left tbody td[name="cell0"]').find('input[checked=true]').data('old',true);
            $('#' + this._gridId() + '-data-left tbody td[name="cell0"]').find('input[checked=true]').attr('checked', false);
        }
    ...
    , _editor: function(event) {
        ...
        if (typeof currCol.checkbox != 'undefined') {
            ...
            tdObj.find('input').bind('change', { grid: grid }, function(event) {
                var val = {};
                val[tdObj.attr('name')] = $(this).attr('checked') ? currCol.checkbox.checked : currCol.checkbox.unchecked;
        ...
        if (tdObj.find('span').length > 0) {
            ...
            $('#editor').bind('dblclick', { grid: event.data.grid }, function(event) {
                event.data.grid._celldblclick(event,{name:tdObj.attr('name'),value:$(this).val()});
            });
            /*
            IE不能及時得到焦點
            */
...
//            gd_body_r.append('<div id="' + tmpId + '" class="ui-widget ui-widget-content" style="overflow: hidden;  position: relative;"></div>');
            ...
            //调整right body起始位置
            gd_body_r.css({ left: this.options.widthLeft + 10 });
        ...
        */ 
        , _makeTitle: function() {
            ...
            tdCond_l = tdCond_r = '<tr name="cond" style="height:18px;display:' + (this.options.condCol ? '' : 'none') + '">';
            ...
            for (var rows = 0; rows < titleRows; rows++) {
                tdTitle_l = tdTitle_r = '<tr style="height: 18px;" class="ui-widget-header" >';
